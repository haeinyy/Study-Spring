...
public class GuestBookDaoImpl implements GuestBookDao {
	...
	public void registerArticle(GuestBookDto guestBookDto) throws Exception {
		...
		try {
			...
			conn.setAutoCommit(false); // autocommit false??? =? file업로드시 요류났는데, 글만 db에 반영되고 file 반영안되는 경우방지하기 위해 
			...
			if(fileInfos != null && !fileInfos.isEmpty()) {
				String lastNo = "select last_insert_id()"; // 글작성시 마지막에 auto_incre 된 글번호 얻어와라 
				...
				for(int i=0;i<size;i++) {
					reigsterFile.append("(?, ?, ?, ?)");
					if(i != fileInfos.size() - 1) // 끝이 아닐때에 
						reigsterFile.append(","); // ,찍기!!
				}
			...
			conn.commit(); // 글작성,파일업로드 둘다 잘 실행되면 커밋!
