...
public class FileDownLoadView extends AbstractView {
	...
	protected void renderMergedOutputModel(Map<String, Object> model, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		String realPath = ctx.getRealPath("/upload"); // 실제 경로 얻어오기
        ...
        File file = new File(realPath + File.separator  + saveFolder, saveFile); // 파일객체 만듬
		
        // content-type을 download type으로 바꿈 
        ...
        // IE는 다르게 처리
        if (isIE) {
        	fileName = URLEncoder.encode(originalFile, "UTF-8").replaceAll("\\+", "%20"); // +를 공백으로 바꿔라
        } else {
            fileName = new String(originalFile.getBytes("UTF-8"), "ISO-8859-1");
        }
        response.setHeader("Content-Disposition", "attachment; filename=\"" + fileName + "\";");
        response.setHeader("Content-Transfer-Encoding", "binary");
        OutputStream out = response.getOutputStream(); // 서버에서 우리한테 output 
